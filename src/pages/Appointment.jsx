...
const Appointment = () => {
  ...
  return (
    docInfo && (
      <div className="relative p-4 pt-12"> {/* added top padding */}
        {/* Back to Home button */}
        <button
          onClick={() => navigate('/home')}
          className="fixed top-4 left-4  cursor-pointer flex items-center gap-1 px-3 py-1 bg-green-800 text-white rounded hover:bg-green-700 transition z-50"
        >
          <Home size={16} />
        ...
        <div className="flex items-center gap-3 w-full overflow-x-scroll mt-4">
          {docSlots.length && docSlots[slotIndex].map((item, index) => (
            <p key={index} onClick={() => setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 cursor-pointer rounded-full ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-900 border border-gray-600'}`}>
              {item.time.toLowerCase()}
            </p>
          ))}
        </div>
        <button className="bg-primary text-white text-sm border rounded-full px-8 py-3 mt-9 text-center">Book an Appointment</button>
